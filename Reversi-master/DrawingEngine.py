...
SCREEN_SIZE = (8 * SMALL_SQUARE_SIZE, 8 * SMALL_SQUARE_SIZE)
    ...
    def draw_board(self):
        ...
        for i in range(1, len(config)):
            for j in range(1, len(config)):
                pygame.draw.rect(self.screen, GREEN, ((i - 1) * SMALL_SQUARE_SIZE, (j - 1) * SMALL_SQUARE_SIZE, SMALL_SQUARE_SIZE, SMALL_SQUARE_SIZE))
                pygame.draw.rect(self.screen, DARK_GREEN, ((i - 1) * SMALL_SQUARE_SIZE, (j - 1) * SMALL_SQUARE_SIZE, SMALL_SQUARE_SIZE, SMALL_SQUARE_SIZE), width=2)
                curr_color = BLACK if self.game_engine.state.current_player == JMAX else WHITE
                if (i, j) in moves:
                    pygame.draw.circle(self.screen, curr_color, ((i - 1) * SMALL_SQUARE_SIZE + SMALL_SQUARE_SIZE / 2, (j - 1) * SMALL_SQUARE_SIZE + SMALL_SQUARE_SIZE / 2), SMALL_SQUARE_SIZE / 2 - 10, width=2)
                if config[i][j] == EMPTY:
                    continue
                color = BLACK if config[i][j] == JMAX else WHITE
                pygame.draw.circle(self.screen, color, ((i - 1) * SMALL_SQUARE_SIZE + SMALL_SQUARE_SIZE / 2, (j - 1) * SMALL_SQUARE_SIZE + SMALL_SQUARE_SIZE / 2), SMALL_SQUARE_SIZE / 2 - 10)
    ...
    def run(self):
        ...
        while running:
            ...
            if self.game_engine.state.is_final():
                ...
                X, Y = SCREEN_SIZE
                self.screen.blit(text, (SMALL_SQUARE_SIZE // 2, Y // 2))
...
if __name__ == '__main__':
    while True:
        try:
            human = input("white or black (w/b)?")
    ...
    while True:
        try:
            algorithm = int(input("Minimax or alpha-beta(1/2)?"))
